...
%%source = aviread('C:\Program Files\MATLAB\R2008a\work\Background_Subtraction_with_MCS\Background_Subtraction_with_MCS\test_video\san_fran_traffic_30sec_QVGA_Cinepak');
...
for n = 2:nf;
    ...
    fg1=im2bw(fg);                  % Convert gray imgate to binary
     fg1=bwmorph(fg1,'bridge');     % Connect all pixels falls within 3 x 3 box
     fg1=bwareaopen(fg1,200);       % Remove areas less than 200 pixels
     fg1=bwmorph(fg1,'dilate');     % Perform dilation
     objects=double(fr).*(cat(3,fg1,fg1,fg1));  % Multiply with mask to obtain objects
     [L, num] = bwlabel(fg1);       % Calculate the total number of areas in mask
    pid=1;
    if originalvideo==1;
